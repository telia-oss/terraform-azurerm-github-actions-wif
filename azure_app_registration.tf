...
resource "azuread_application" "github_oidc" {
  for_each = { for env in local.environments_to_create_apps : env.environment => env }
...
data "azuread_application" "existing" {
  for_each       = { for env in local.environments_to_reference_apps : env.environment => env }
  application_id = try(each.value.application_id, "")
}

# The Azure Active Directory service principal resource is used to create the GitHub OIDC app service principal
resource "azuread_service_principal" "github_oidc" {
  for_each       = zipmap([for env in local.environments_to_create_apps : env.environment], [for env in local.environments_to_create_apps : env])
  application_id = azuread_application.github_oidc[each.value.environment].application_id
}

# The Azure Active Directory application resource is used to create the GitHub OIDC app
resource "azuread_application_federated_identity_credential" "github_oidc" {
  for_each              = { for e in local.repo_environments : "${e.repository_name}-${e.environment}" => e }
  application_object_id = try(local.environments_to_create_apps[each.key] != null ? azuread_application.github_oidc[each.value.environment].object_id : null, data.azuread_application.existing[each.value.environment].object_id)
